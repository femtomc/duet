...
# Define channels
issue = Channel(name="issue", schema="text", description="Bug report or feature request")
diagnosis = Channel(name="diagnosis", schema="text", description="Triage assessment")
fix_description = Channel(name="fix_description", schema="text", description="Implementation summary")
test_result = Channel(name="test_result", schema="verdict", description="QA verdict")
notes = Channel(name="notes", schema="text", description="Feedback notes")
...
success = Phase(name="success", agent="qa_agent", is_terminal=True)
blocked = Phase(name="blocked", agent="qa_agent", is_terminal=True)
...
workflow = Workflow(
    ...
    channels=[issue, diagnosis, fix_description, test_result, notes],
    phases=[triage, fix, qa, success, blocked],
    transitions=[
